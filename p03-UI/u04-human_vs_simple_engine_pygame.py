...
class ChessGUI:
    def __init__(self):
        ...
        pygame.display.set_caption("Human vs Simple Engine - Chess")
        self.clock = pygame.time.Clock()
        self.board = chess.Board()
        self.engine = SimpleEngine(depth=3)
        
        # Use freetype fonts with better Unicode support
        # Try common fonts that support chess symbols
        font_names = ['Segoe UI Symbol', 'Arial Unicode MS', 'DejaVu Sans', 'FreeSans', 'Arial']
        self.font_pieces = None
        for font_name in font_names:
            try:
                self.font_pieces = pygame.freetype.SysFont(font_name, 60)
                break
            except:
                continue
        
        if self.font_pieces is None:
            # Fallback to default
            self.font_pieces = pygame.freetype.Font(None, 60)
    ...
    def draw_pieces(self):
        ...
        for square in chess.SQUARES:
            ...
            if piece:
                ...
                text, text_rect = self.font_pieces.render(symbol, color)
